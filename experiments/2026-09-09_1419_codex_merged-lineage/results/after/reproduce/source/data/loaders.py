...
class BindingRecord:
    ...
    # Publication/observation provenance stays outside predictive inputs.
    doi: str = ""
    reference_text: str = ""
...
class ElutionRecord:
    ...
    mapping_proteome_source: str = ""
    mapping_is_canonical_transcript: Optional[bool] = None
    #: Complete allele set reported or generated by the source observation.
    #: Resolution may narrow `alleles` for model input, but must never mutate
    #: this immutable provenance copy. Appended after the older fields to
    #: preserve their positional constructor contract.
    source_alleles: Tuple[str, ...] = ()
    # Publication/observation provenance stays outside predictive inputs.
    doi: str = ""
    reference_text: str = ""
        ...
        for rec in processing_records or []:
            ...
            self.samples.append(
                PrestoSample(
                    ...
                    primary_allele=rec.mhc_allele,
                    synthetic_kind=_synthetic_kind(rec.source),
                    **source_lineage_fields(rec),
                    sample_id=_sample_id("proc", rec),
                )
            )
        ...
        for rec in tcell_records or []:
            ...
            self.samples.append(
                PrestoSample(
                    ...
                    synthetic_kind=_synthetic_kind(rec.source),
                    **source_lineage_fields(rec),
                    sample_id=_sample_id("tcell", rec),
                )
            )
        ...
        for rec in tcr_evidence_records or []:
            ...
            self.samples.append(
                PrestoSample(
                    ...
                    synthetic_kind=_synthetic_kind(rec.source),
                    **source_lineage_fields(rec),
                    sample_id=_sample_id("tcrev", rec),
                )
            )
    ...
    for _ in range(n_samples):
        ...
        records.append(
            ElutionRecord(
                ...
                source_alleles=tuple(sample_alleles),
