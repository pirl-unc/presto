#!/usr/bin/env python
"""Launch a matched local CPU vs MPS all-class-I PF07 confirmation under default dropout mode."""
...
DEFAULT_PREFIX = "presto-allclass1-device-auto-20260320a"
...
VARIANTS = (
    {"condition_key": "cpu", "description": "Matched all-class-I reduced PF07 validation on CPU under default auto mode", "device": "cpu"},
    {"condition_key": "mps", "description": "Matched all-class-I reduced PF07 validation on Apple Silicon MPS under default auto mode", "device": "mps"},
)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run matched local CPU vs MPS all-class-I PF07 confirmation under default auto dropout mode.")
    ...
    if int(args.max_records) <= 0:
        raise SystemExit("--max-records must be positive for this reduced confirmation family")
    ...
    metadata = {
        ...
        "training": {
            "pretraining": {"mode": "mhc_pretrain", "warm_start_checkpoint_local": _portable_path_str(init_checkpoint)},
            "downstream": {
    ...
    out_dir = initialize_experiment_dir(
        ...
        slug="cpu-vs-mps-allclass1-auto-default-confirmation",
        title="CPU vs MPS All-Class-I Auto-Default Confirmation",
        source_script="experiments/2026-03-20_0920_codex_cpu-vs-mps-allclass1-auto-default-confirmation/code/launch.py",
    ...
    for variant in VARIANTS:
        ...
        if args.dry_run:
            launched.append({"condition_key": condition_key, "device": device, "run_id": run_id, "command": cmd})
            continue
    ...
    if failures:
        raise SystemExit(json.dumps({"out_dir": str(out_dir), "n_completed": len(launched) - len(failures), "n_failed": len(failures), "failures": failures, "manifest": str(manifest_path)}, indent=2))
    print(json.dumps({"out_dir": str(out_dir), "n_completed": len(launched), "manifest": str(manifest_path)}, indent=2))
