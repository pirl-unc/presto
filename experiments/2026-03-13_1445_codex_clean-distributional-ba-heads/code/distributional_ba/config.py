"""Fixed 12-condition matrix and model factory for the clean BA benchmark."""
...
class ConditionSpec:
    """One cell of the fixed 12-condition matrix."""
...
def _make_conditions() -> List[ConditionSpec]:
    ...
    for max_nM in (50_000, 200_000):
        _append("mhcflurry", "additive", max_nM)
    for max_nM in (50_000, 200_000):
        _append("log_mse", "additive", max_nM)
    for max_nM in (50_000, 200_000):
        for n_bins in (64, 128):
            _append("twohot", "d2_logit", max_nM, n_bins=n_bins)
    for max_nM in (50_000, 200_000):
        for n_bins in (64, 128):
            _append("hlgauss", "d2_logit", max_nM, n_bins=n_bins, sigma_mult=0.75)

    assert len(conds) == 12, f"Expected 12 conditions, got {len(conds)}"
    return conds
