...
def main() -> None:
    ...
    for entry in manifest:
        ...
        final_probe_df.insert(2, "warm_start", bool(entry.get("warm_start", False)))
        ...
        for epoch_summary in epoch_summaries:
            row = {
                ...
                "warm_start": bool(entry.get("warm_start", summary["config"].get("warm_start_requested", False))),
    ...
    warm_start_summary_df = (
        condition_df.groupby(["head_type", "warm_start"], sort=False)[
            ["test_spearman", "test_rmse_log10", "test_auroc", "test_auprc", "mean_epoch_time_s"]
        ]
        .mean()
        .reset_index()
        .sort_values(["head_type", "warm_start"])
    )
    cap_summary_df = (
    ...
    warm_start_summary_df.to_csv(RESULTS_DIR / "warm_start_summary.csv", index=False)
    ...
    warm_start_md = markdown_table(
        warm_start_summary_df,
        ["head_type", "warm_start", "test_spearman", "test_rmse_log10", "test_auroc", "test_auprc", "mean_epoch_time_s"],
    )
    (RESULTS_DIR / "topline_test_ranking.md").write_text(topline_md + "\n")
    (RESULTS_DIR / "family_summary.md").write_text(family_md + "\n")
    (RESULTS_DIR / "warm_start_summary.md").write_text(warm_start_md + "\n")
    ...
    summary_bundle = {
        ...
        "warm_start_summary": warm_start_summary_df.to_dict(orient="records"),
