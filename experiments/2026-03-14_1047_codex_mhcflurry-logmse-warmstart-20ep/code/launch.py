#!/usr/bin/env python
"""Launch the self-contained 4-condition regression warm-start benchmark on Modal."""
...
DEFAULT_GPU = "H100!"
DEFAULT_WARM_START_CHECKPOINT = "/checkpoints/mhc-pretrain-20260308b/mhc_pretrain.pt"
    ...
    *,
    ...
    return {
        ...
        "training": {
            ...
            "warm_start_source": DEFAULT_WARM_START_CHECKPOINT,
            "warm_start_mode": "partial_encoder_only",
        },
        "tested": [
            {
...
def _launch_condition(
    *,
    ...
    if spec.warm_start:
        extra_args_parts.extend(["--init-checkpoint", DEFAULT_WARM_START_CHECKPOINT])

    cmd = [
        "modal",
        "run",
        "--detach",
        "scripts/train_modal.py::distributional_ba_regwarm_run",
        "--cond-id",
        str(spec.cond_id),
        "--epochs",
        str(int(epochs)),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Launch the 4-condition regression warm-start sweep on Modal")
    ...
    parser.add_argument("--prefix", type=str, default="dist-ba-regwarm-20ep")
    ...
    parser.add_argument("--smoke", action="store_true", help="Launch only warm-start mhcflurry with 1 epoch and a small record cap.")
    ...
    out_dir = initialize_experiment_dir(
        ...
        slug="mhcflurry-logmse-warmstart-20ep",
        title="MHCflurry / LogMSE Cold vs Warm Start",
