#!/usr/bin/env python
"""Launch 6-condition v4 fine-grained embed_dim sweep on Modal.

Usage:
    python scripts/benchmark_distributional_ba_heads_v4.py
    python scripts/benchmark_distributional_ba_heads_v4.py --cond-ids 1,3,6
    python scripts/benchmark_distributional_ba_heads_v4.py --dry-run
"""
...
from presto.scripts.distributional_ba.config_v4 import CONDITIONS_V4, ConditionSpec
...
def _launch_condition(
    *,
    ...
    cmd = [
        ...
        "run",
        "--detach",
        "scripts/train_modal.py::distributional_ba_v4_run",
        "--cond-id",
        str(spec.cond_id),
        "--epochs",
        str(epochs),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Launch 6-condition v4 fine-grained embed_dim sweep on Modal",
    )
    ...
    parser.add_argument("--prefix", type=str, default="dist-ba-v4")
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Optional comma-separated subset of condition IDs (1-6).",
    )
    ...
    selected_conditions = [
        spec for spec in CONDITIONS_V4 if not selected_ids or spec.cond_id in selected_ids
    ]
    ...
    out_dir = initialize_experiment_dir(
        ...
        slug="embed-dim-fine-sweep",
        title="Fine-Grained Encoder Dimension Sweep (6 conditions, 50 epochs)",
        source_script="scripts/benchmark_distributional_ba_heads_v4.py",
