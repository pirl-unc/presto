#!/usr/bin/env python
"""Launch 32-condition distributional vs regression BA head experiment on Modal.

Usage:
    python scripts/benchmark_distributional_ba_heads.py
    python scripts/benchmark_distributional_ba_heads.py --cond-ids 1,5,7
    python scripts/benchmark_distributional_ba_heads.py --dry-run
"""
...
from presto.scripts.distributional_ba.config import CONDITIONS, ConditionSpec
...
def _launch_condition(
    *,
    ...
    cmd = [
        ...
        "run",
        "--detach",
        "scripts/train_modal.py::distributional_ba_run",
        "--cond-id",
        str(spec.cond_id),
        "--epochs",
        str(epochs),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Launch 32-condition distributional BA head experiments on Modal",
    )
    ...
    parser.add_argument("--prefix", type=str, default="dist-ba-v1")
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Optional comma-separated subset of condition IDs (1-32).",
    )
    ...
    selected_conditions = [
        spec for spec in CONDITIONS if not selected_ids or spec.cond_id in selected_ids
    ]
    ...
    out_dir = initialize_experiment_dir(
        ...
        slug="distributional-ba-heads",
        title="Distributional vs Regression BA Heads (32 conditions)",
        source_script="scripts/benchmark_distributional_ba_heads.py",
