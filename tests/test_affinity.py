...
def test_affinity_nm_to_log10_clamps_to_global_max():
    ...
    assert torch.allclose(converted[1], torch.tensor(5.0), atol=1e-6)
...
def test_normalize_binding_target_log10_auto_detects_nm_and_log10():
    ...
    assert torch.allclose(
        ...
        torch.tensor([math.log10(50000.0), 5.0]),
        atol=1e-5,
    )

    log_values = torch.tensor([[4.7], [5.3]], dtype=torch.float32)
    log_normalized = normalize_binding_target_log10(log_values)
    assert torch.allclose(log_normalized[0], torch.tensor([4.7]), atol=1e-6)
    assert torch.allclose(log_normalized[1], torch.tensor([5.0]), atol=1e-6)
