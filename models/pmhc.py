...
class MSDetectionHead(nn.Module):
    """Mass spectrometry detection head.

    Predicts MS detectability, which is related to but not identical to
    biological presentation (includes ionization efficiency, etc.).
    """

    def __init__(self, d_model: int = 256, aa_vocab_size: int = 30):
        super().__init__()
        # MHC-conditioned context branch (presentation-related sequence context).
        self.context_head = nn.Sequential(
            nn.Linear(d_model, d_model // 2),
            nn.GELU(),
            nn.Linear(d_model // 2, 1),
        )
        # Peptide-only detectability branch (ionization/physchem propensity).
        self.peptide_embed = nn.Embedding(aa_vocab_size, d_model)
        self.peptide_head = nn.Sequential(
            nn.Linear(d_model, d_model // 2),
            nn.GELU(),
            nn.Linear(d_model // 2, 1),
        )
        # Dominant biologic drivers for observed MS ligand signal.
        self.w_proc = nn.Parameter(torch.tensor(1.2))
        self.w_pres = nn.Parameter(torch.tensor(1.4))
        self.w_context = nn.Parameter(torch.tensor(0.4))
        self.w_peptide = nn.Parameter(torch.tensor(0.8))
        self.bias = nn.Parameter(torch.zeros(1))
    ...
    def forward(
        ...
        pep_tok: torch.Tensor,
        proc_logit: torch.Tensor,
        pres_logit: torch.Tensor,
    ) -> torch.Tensor:
        """Predict MS detection logit.

        Args:
            pmhc_vec: pMHC vector representation (batch, d_model)
            pep_tok: peptide tokens (batch, pep_len)
            proc_logit: processing logit (batch, 1)
            pres_logit: presentation logit (batch, 1)

        Returns:
            Detection logit (batch, 1)
        """
        context_vec = self.context_head(pmhc_vec)
        pep_emb = self.peptide_embed(pep_tok)
        pep_mask = (pep_tok != 0).float().unsqueeze(-1)
        pep_sum = (pep_emb * pep_mask).sum(dim=1)
        pep_len = pep_mask.sum(dim=1).clamp(min=1.0)
        pep_vec = pep_sum / pep_len
        pep_logit = self.peptide_head(pep_vec)

        w_proc = F.softplus(self.w_proc)
        w_pres = F.softplus(self.w_pres)
        w_ctx = F.softplus(self.w_context)
        w_pep = F.softplus(self.w_peptide)
        return (
            w_proc * proc_logit
            + w_pres * pres_logit
            + w_ctx * context_vec
