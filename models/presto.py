...
from .heads import AssayHeads, TCellAssayHead, ElutionHead, CategoryHead
...
class Presto(nn.Module):
    ...
    LATENT_SEGMENTS = {
        ...
        "recognition_cd8": ["peptide"],  # peptide only
        "recognition_cd4": ["peptide"],  # peptide only
    ...
    LATENT_DEPS = {
        ...
        "recognition_cd8": [],   # no latent deps, peptide-only cross-attention
        "recognition_cd4": [],   # no latent deps, peptide-only cross-attention
    ...
    def __init__(
        ...
        self.mhc_class_cond_embed = nn.Embedding(3, d_model)  # {I, II, unknown}
        ...
        # Per-chain type: {class_I_heavy, class_II_alpha, beta2m, unknown}
        self.mhc_a_type_head = nn.Linear(d_model, 4)
        self.mhc_b_type_head = nn.Linear(d_model, 4)
        # Per-chain species
        ...
        self.chain_compat_head = nn.Sequential(
            nn.Linear(d_model * 2 + 4 + 4 + n_species * 2, d_model),
            nn.GELU(),
            nn.Linear(d_model, 1),
        )
        ...
        # Optional UniProt category head.
        self.category_head = None
        if n_categories is not None:
            self.category_head = CategoryHead(d_model=d_model, n_categories=n_categories)
    ...
    def _ensure_optional_segment(
        ...
        if tok is None:
            return torch.ones((batch_size, 1), dtype=torch.long, device=device)
    ...
    def _build_single_stream(
        ...
        mhc_class_id: Optional[torch.Tensor] = None,
        has_tcr: bool = False,
        ...
        if mhc_class_id is None:
            mhc_class_id = torch.full((batch_size,), 2, dtype=torch.long, device=device)  # 2 = unknown
        ...
        global_cond = (
            self.species_cond_embed(species_id)
            + self.mhc_class_cond_embed(mhc_class_id)
    ...
    def forward(
        ...
        # ------------------------------------------------------------------
        # Resolve species_id for global conditioning
        _species_map = {s: i for i, s in enumerate(PROCESSING_SPECIES_BUCKETS)}
        if isinstance(species, str):
            _sp = normalize_processing_species_label(species, default="other")
            species_id = torch.full((pep_tok.shape[0],), _species_map.get(_sp, 3), dtype=torch.long, device=pep_tok.device)
        else:
            species_id = None  # will default to 0 (human) in _build_single_stream

        # Resolve mhc_class_id: 0=I, 1=II, 2=unknown
        if isinstance(mhc_class, str):
            _cls = normalize_mhc_class(mhc_class, default="unknown")
            _cls_id = {"I": 0, "II": 1}.get(_cls, 2)
            mhc_class_id = torch.full((pep_tok.shape[0],), _cls_id, dtype=torch.long, device=pep_tok.device)
        else:
            mhc_class_id = None  # will default to 2 (unknown)

        has_tcr = tcr_a_tok is not None or tcr_b_tok is not None
        has_tcr_paired = tcr_a_tok is not None and tcr_b_tok is not None
        ...
        # Class I = a=class_I_heavy(0) * b=beta2m(2)
        # Class II = a=class_II_alpha(1) * b=class_II_beta(1)
        # (b_type categories: class_I_heavy=0, class_II_beta=1, beta2m=2, unknown=3)
        class1_prob_raw = mhc_a_type_probs[:, 0] * mhc_b_type_probs[:, 2]
        class2_prob_raw = mhc_a_type_probs[:, 1] * mhc_b_type_probs[:, 1]
        ...
        # ------------------------------------------------------------------
        # 4) Optional TCR encoding
        # ------------------------------------------------------------------
        tcr_vec = None
        if tcr_a_tok is not None or tcr_b_tok is not None:
        ...
        for name in self.CROSS_ATTN_LATENTS:
            ...
            latent_vals[name] = self._latent_query(
                name=name,
                h=h,
        ...
        # ------------------------------------------------------------------
        # 10) Optional TCR-specific matching
        # ------------------------------------------------------------------
        ...
        if self.category_head is not None:
            outputs["category_logits"] = self.category_head(pmhc_vec)
